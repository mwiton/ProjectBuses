...
private:
	...
	int time;
	double price;
	static constexpr double PRICE_FOR_KM = 0.25;
public:
	BusLine(std::string name = "", std::string start = "", std::string end = "", int len = 0, int t = 0) :
		nameOfLine(name), startStop(start), endStop(end), length(len), time(t) { price = PRICE_FOR_KM * length; }
...
void createFromInput(TablePointers<BusLine>&);
